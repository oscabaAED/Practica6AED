...
Datos %>% 
  ...
  geom_vline(aes(xintercept = mean(valor, na.rm = TRUE)),
             color = "blue", 
             linetype = "dashed", 
             size = 1)
```
...
frecuencias_ALG <- table(Datos$ALG.cat)
print("Frecuencias para ALG:")
print(frecuencias_ALG)

tabla_contingencia <- table(ALG = Datos$ALG.cat, ANM = Datos$ANM.cat)
print("Tabla de Contingencia ALG vs ANM:")
print(tabla_contingencia)

mosaicplot(tabla_contingencia, 
           main = "Mosaic Plot - ALG vs ANM", 
           color = TRUE,
           las = 1)
```
...
Datos %>% 
  mutate(sqrtALG.num = sqrt(ALG.num)) %>% 
  select(ALG.num, sqrtALG.num, MD.num) %>% 
  cor(use = "complete.obs", method ="pearson")
```
